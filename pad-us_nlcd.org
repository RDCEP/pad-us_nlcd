...
#+begin_src R
  ...
  countyAreas <-
    statsNbcdCounty[, list( totHa= sum(ha)),
                    by= c( "state", "county")]
  setkey( countyAreas, state, county)
  statsNbcdCounty <-
  ...
  statsNbcdCounty <-
    rbind(
      statsNbcdCounty[ !nlcd %in% as.character( c( 41, 42, 43)),
                      list( state, county, nlcd, gap, aldb, ha, fips)],
      statsNbcdCounty[  nlcd %in% as.character( c( 41, 42, 43)),
                      list( state, county, nlcd, gap, aldb= aldb2, ha, fips)])
  setkey( statsNbcdCounty, state, county, nlcd, gap)
  
  zeroCarbonForestsIndex <-
    with(
      statsNbcdCounty,
      nlcd %in% as.character( c( 41, 42, 43)) & is.na( aldb))
  ...
  statsNbcdCounty <-
    rbind(
      ...
      merge(
        statsNbcdCounty[ zeroCarbonForestsIndex],
        statsNbcdState,
        all.x= TRUE)[, list( state, county, nlcd, gap, aldb= aldb.y, ha = ha.x, fips)])
  setkey( statsNbcdCounty, state, county, nlcd, gap)
